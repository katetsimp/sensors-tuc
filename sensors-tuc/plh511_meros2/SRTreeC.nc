...
{ // hold current round
   ...
   // routing message and mess for sending
    ...
    {      uint16_t seedNum;
            ...
            if(TOS_NODE_ID==0)
        {
            ...
            dbg("Boot", "curdepth = %d  ,  parentID= %d ", curdepth , parentID);
    ...
    {       routingwith2func * r2f;
        ...
        dbg("SRTreeC", "RoutingMsgTimer fired!");
          ...
          t1=call RoutingMsgTimer.getNow();
        ...
        //max or count(c=0) or max&count(c=1)  
               routingcounter++; //counter that count how many times doing routing
   ...
   while( measurment>prev_measurment+((float) 10/100) *prev_measurment ||measurment<prev_measurment-((float) 10/100) *prev_measurment){
      ...
      //Phase 2 of project
       //c from 0 to 9
       c=((call randomN.rand16())%10);
         
         
      dbg("Measurments","############ROUND  %u ########\n",roundCounter);
       //c=1  to have 10% possibility
       if( c==1){
        roundCounter--;
         // take a new function 
        call RoutingMsgTimer.startOneShot(TIMER_FAST_PERIOD);
 ...
 dbg("SRTreeC" , "Package sent %s \n", (err==SUCCESS)?"True":"False");
    ...
    task void receiveRoutingTask()
    {
        ...
        if(len == sizeof(routingwith2func)||len == sizeof(routingwith1func))
        {
            ...
            // for second or more rounds
              else{ 
             
                  
                   if((prev_length!=len ||(prev_length==3 && len==3 && prev_c!=choice[0])) &&  TOS_NODE_ID!=0){
